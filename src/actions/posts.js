...
import { FETCH_POST, FETCH_ALL, FETCH_SEARCHED, FETCH_SEARCHED_AUTHOR, START_LOADING, END_LOADING, CREATE, UPDATE, DELETE, LIKE, COMMENT, DELETE_COMMENT, LIKE_COMMENT } from '../constants/actionTypes';
...
export const likeComment = (id, commentId) => async (dispatch) => {
    try {
        const { data: { comments } } = await api.likeComment(id, commentId);
        dispatch({ type: LIKE_COMMENT, payload: comments });
    } catch (error) {
        console.log(error);
    }
}